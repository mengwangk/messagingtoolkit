...
namespace MessagingToolkit.Barcode.Demo
{
    ...
    public partial class frmMain : Form
    {
        ...
        private void InitializeApp()
        {
            ...
            // Populate the character set
            cboAztecCharacterSet.Items.AddRange(CharacterSets.Keys.ToArray());
            cboAztecCharacterSet.SelectedIndex = 0;
        ...
        private void btnPdf417Encode_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtPdf417Data.Text))
            {
                try
                {
                    ...
                    barcodeEncoder.AddOption(EncodeOptions.Margin, Convert.ToInt32(numPdf417QuietZone.Value));
        ...
        private void btnBrowseFile_Click(object sender, EventArgs e)
        {
            ...
            if (fd.ShowDialog() == DialogResult.OK)
            {
                ...
                txtBarcodeResultPoint.Text = string.Empty;
            }
        }
            ...
            try
            {
                if (!chkMultipleBarcodes.Checked)
                {
                    ...
                    if (decodedResult != null)
                    {
                        ...
                        foreach (ResultPoint resultPoint in  decodedResult.ResultPoints) 
                        {
                            result += "X: " + resultPoint.X + ", Y: " + resultPoint.Y + "\r\n";
                        }
                        txtBarcodeResultPoint.Text = result;
                ...
                else
                {
                    ...
                    foreach (Result decodedResult in results)
                    {
                        content += "Barcode Type: " + decodedResult.BarcodeFormat + Environment.NewLine;
                        content += "Content: " + decodedResult.Text + Environment.NewLine + Environment.NewLine;
                        ...
                        txtBarcodeResultPoint.AppendText(result);
        ...
        /// <returns></returns>
        private Bitmap ResizeImage(Image img, int percentage)
        {
            ...
            int resizedW = (int)(originalW * percentage / 100.0);
            int resizedH = (int)(originalH * percentage / 100.0);
        ...
        private void btnResize_Click(object sender, EventArgs e)
        {
            try
            {
                bitmap = ResizeImage(bitmap, (int)nudImageSize.Value);
                picBoxDecode.Image = (Image)bitmap;
            }
            catch (Exception)
            {
                MessageBox.Show("Unable to resize", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }


        private void btnGenerateQRCodeSVG_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtQRCodeData.Text))
            {
                try
                {
                    ...
                    svgProvider.Foreground = btnQRCodeForeColor.BackColor;
                    svgProvider.Background = btnQRCodeBackColor.BackColor;
                    Svg svg = barcodeEncoder.Generate(BarcodeFormat.QRCode, txtQRCodeData.Text, svgProvider);

                    string tempPath = Path.GetTempPath();
                    string fileName = tempPath + Path.DirectorySeparatorChar + "sample.svg";
                    File.WriteAllText(fileName, svg.Content);
                    MessageBox.Show("Barcode saved to " + fileName, Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private void btnGenerateQRCodeEPS_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtQRCodeData.Text))
            {
                try
                {
                    ...
                    epsProvider.Foreground = btnQRCodeForeColor.BackColor;
                    epsProvider.Background = btnQRCodeBackColor.BackColor;
                    Eps eps = barcodeEncoder.Generate(BarcodeFormat.QRCode, txtQRCodeData.Text, epsProvider);

                    string tempPath = Path.GetTempPath();
                    string fileName = tempPath + Path.DirectorySeparatorChar + "sample.eps";
                    File.WriteAllText(fileName, eps.Content);
                    MessageBox.Show("Barcode saved to " + fileName, Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private void btnAztecEncode_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtAztecData.Text))
            {
                try
                {
                    
                    barcodeEncoder.ForeColor = btnAztecForeColor.BackColor;
                    barcodeEncoder.BackColor = btnAztecBackColor.BackColor;
                    barcodeEncoder.Width = (int)numAztecWidth.Value;
                    barcodeEncoder.Height = (int)numAztecHeight.Value;

                    // Set character set
                    barcodeEncoder.CharacterSet = CharacterSets[cboAztecCharacterSet.Text];

                    // Set error correction level
                    Dictionary<EncodeOptions, object> encodingOptions = new Dictionary<EncodeOptions, object>(1);
                    encodingOptions.Add(EncodeOptions.ErrorCorrection, (int)nudAztecErrorCorrection.Value);

                    picEncodedAztech.Image = barcodeEncoder.Encode(BarcodeFormat.Aztec, txtAztecData.Text, encodingOptions);
                }
            ...
            {
                MessageBox.Show("Please specify the Aztec content", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void btnAztecSave_Click(object sender, EventArgs e)
        {
            Bitmap bm = (Bitmap)picEncodedAztech.Image;
            SaveImage(bm);
        }

        private void btnAztecForeColor_Click(object sender, EventArgs e)
        {
            colorDialog1.AllowFullOpen = true;
            colorDialog1.ShowHelp = true;
            colorDialog1.Color = this.btnAztecForeColor.BackColor;
            if (colorDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                this.btnAztecForeColor.BackColor = colorDialog1.Color;
            }
        }

        private void btnAztecBackColor_Click(object sender, EventArgs e)
        {
            colorDialog1.AllowFullOpen = true;
            colorDialog1.ShowHelp = true;
            colorDialog1.Color = this.btnAztecBackColor.BackColor;
            if (colorDialog1.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                this.btnAztecBackColor.BackColor = colorDialog1.Color;
            }
        }
