...
namespace MessagingToolkit.Barcode.Windows8.ClassLibraryDemo.Helper
{
    public static class WriteableBitmapLoadExtensions
    {
        public static async Task<WriteableBitmap> LoadAsync(string relativePath)
        {
        ...
        public static async Task<WriteableBitmap> LoadAsync(StorageFile file)
        {
            return await new WriteableBitmap(1, 1).LoadAsync(file);
        }

        public static async Task<WriteableBitmap> LoadAsync(StorageFile file, uint decodePixelWidth, uint decodePixelHeight)
        {
            return await new WriteableBitmap(1, 1).LoadAsync(file, decodePixelWidth, decodePixelHeight);
        }

        public static async Task<WriteableBitmap> LoadAsync(this WriteableBitmap writeableBitmap, string relativePath)
        {
