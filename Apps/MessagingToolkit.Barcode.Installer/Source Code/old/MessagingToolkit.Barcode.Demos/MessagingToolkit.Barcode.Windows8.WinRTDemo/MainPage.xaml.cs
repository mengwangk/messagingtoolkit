...
using MessagingToolkit.Barcode.Windows8.WinRTDemo.Helper;


// The Basic Page item template is documented at http://go.microsoft.com/fwlink/?LinkId=234237

namespace MessagingToolkit.Barcode.Windows8.WinRTDemo
{
    /// <summary>
    /// A basic page that provides characteristics common to most applications.
    /// </summary>
    public sealed partial class MainPage : MessagingToolkit.Barcode.Windows8.WinRTDemo.Common.LayoutAwarePage
    {
        private WriteableBitmap barcodeImage = null;
        private StorageFile barcodeFile = null;
        ...
        private async void btnEncode_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Perform encoding
                IRandomAccessStream stream = await barcodeEncoder.Encode(format, text);

                // Generate WritableBitmap
                BitmapDecoder decoder = await BitmapDecoder.CreateAsync(stream);
                this.barcodeImage = new WriteableBitmap((Int32)decoder.PixelWidth, (Int32)decoder.PixelHeight);
                stream.Seek(0);
                this.barcodeImage.SetSource(stream);
        ...
        private async void btnGenerateSVG_Click(object sender, RoutedEventArgs e)
        {
            if (!string.IsNullOrEmpty(txtBarcodeContent.Text))
            {
                try
                {
                    ...
                    IOutput svg = barcodeEncoder.Generate(format, txtBarcodeContent.Text, svgProvider);
        ...
        private async void btnBrowse_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                this.barcodeFile = await openPicker.PickSingleFileAsync();

                if (this.barcodeFile != null)
                {
                    // Application now has read/write access to the picked file
                    WriteableBitmap bitmap = await WriteableBitmapLoadExtensions.LoadAsync(this.barcodeFile);
                    txtDecodeImage.Text = this.barcodeFile.Name;
        ...
        private async void btnGenerateEPS_Click(object sender, RoutedEventArgs e)
        {
            if (!string.IsNullOrEmpty(txtBarcodeContent.Text))
            {
                try
                {
                    ...
                    IOutput svg = barcodeEncoder.Generate(format, txtBarcodeContent.Text, epsProvider);
