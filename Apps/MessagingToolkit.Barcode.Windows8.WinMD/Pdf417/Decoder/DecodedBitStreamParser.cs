...
#if !NET35 && !WINDOWS_PHONE && !SILVERLIGHT3
...
namespace MessagingToolkit.Barcode.Pdf417.Decoder
{
    ...
    internal sealed class DecodedBitStreamParser
    {
        ...
        static internal DecoderResult Decode(int[] codewords, String ecLevel, int errorCorrectionCount)
        {
            StringBuilder result = new StringBuilder(codewords.Length * 2);
            // Get compaction mode
            int codeIndex = 1;
            int code = codewords[codeIndex++];
            MacroPdf417Block macroPdf417Block = null;
            while (codeIndex < codewords[0])
            {
                switch (code)
                {
                    ...
                    case BEGIN_MACRO_PDF417_CONTROL_BLOCK:
                        ...
                        codeIndex = DecodeMacroBlock(codewords, codeIndex, macroPdf417Block);
                        break;
                    default:
                        // Default to text compaction. During testing numerous barcodes
                        // appeared to be missing the starting mode. In these cases defaulting
            ...
            return new PdfDecoderResult(null, result.ToString(), null, ecLevel, errorCorrectionCount, macroPdf417Block);
        }

        private static int DecodeMacroBlock(int[] codewords, int codeIndex, MacroPdf417Block macroPdf417Block)
        {
            bool end = false;
            ...
            macroPdf417Block.SegmentIndex = Convert.ToInt32(DecodeBase900toBase10(segmentIndexArray, NUMBER_OF_SEQUENCE_CODEWORDS));

            StringBuilder fileId = new StringBuilder();
            codeIndex = TextCompaction(codewords, codeIndex, fileId);
            macroPdf417Block.FileId = fileId.ToString();
            ...
            if (BEGIN_MACRO_PDF417_OPTIONAL_FIELD == codewords[codeIndex])
            {
                ...
                int upper = additionalOptionCodeWordsIndex;
                if (upper > additionalOptionCodeWords.Length)
                    upper = additionalOptionCodeWords.Length;
                int[] data = new int[upper];
                for (int i = 0; i < upper; i++)
                    data[i] = additionalOptionCodeWords[i];

                macroPdf417Block.OptionalData = data;
            }
