...
namespace MessagingToolkit.Barcode.Pdf417.Encoder
{
    ...
    internal sealed class Pdf417
    {
        ...
        public const float PreferredRatio = 3.0f;
        ...
        private void EncodeLowLevel(string fullCodewords, int c, int r, int errorCorrectionLevel, BarcodeMatrix logic)
        {
            ...
            for (int y = 0; y < r; y++)
            {
                ...
                if (compact)
                {
                    EncodeChar(StopPattern, 1, logic.GetCurrentRow()); // encodes stop line for compact pdf417
                }
                else
                {
        ...
        /// </returns>
        /// <exception cref="BarcodeEncoderException">Unable to fit message in columns</exception>
        internal int[] DetermineDimensions(int sourceCodeWords, int errorCorrectionCodeWords)
        {
            float ratio = 0.0f;
            int[] dimension = null;
            ...
            for (int cols = minCols; cols <= maxCols; cols++)
            {
                ...
                float newRatio = ((17 * cols + 69) * DefaultModuleWidth) / (rows * Height);
            ...
            if (dimension == null)
            {
                int rows_0 = CalculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, minCols);
                if (rows_0 < minRows)
                {
        ...
        /// <summary>
        /// Sets compaction to values stored in <see cref="T:Com.Google.Zxing.Pdf417.Encoder.Compaction"/> enum
        /// </summary>
        ///
