...
import com.mbit.valueobject.image.ImageFilter;
import com.mbit.valueobject.image.ImageRecord;
import com.mbit.browser.*;
 ...
 *         Created : Oct 3, 2006 11:03:32 PM
 * @version : $Id:
 */
public class BrowserItem extends CustomItem implements CommandListener {

    public static final String HOMEPAGE_URL = "resource://index.html";

    private BaseMidlet app;
    private String httpMimeType;

    private Hashtable imageTable = new Hashtable();
    private Hashtable cacheImageTable = new Hashtable();
    ...
    // command
    static final Command pgUp = new Command("PgUp", Command.BACK, 30);
    static final Command pgDn = new Command("PgDn", Command.SCREEN, 40);
    ...
    public BrowserItem(BaseMidlet app) {
    ...
    public void setText(String text) {
        ...
        try {
            ...
            byte[] unicode = text.getBytes(Constants.PAGE_ENCODING);

            String s1 = new String(ascii);
            String s2 = new String(unicode);
                  ...
                  */
                    ...
                    if (overrun && (buf.length() > 0)) {
                        if (wordWrap) {
                            ...
                            //System.out.println("whitespace " + lastWhitespaceIdx);
                   ...
                   + ": "+currentItem.text
                    ...
                    // the linewrap
                    /*
                    System.out.println("elt length: " + elt.value.length());
    ...
    void paintImageItem(Graphics g, HtmlItem item, Line line) {
        ...
        if (item.image == null) {
            g.drawString("[ERROR:NULLIMAGE=" + lines.indexOf(line)
                    + ":" + items.indexOf(item)
                    + "]",
                    item.xpos, line.ypos - nPosY, Graphics.LEFT | Graphics.BOTTOM);
        } else {
    ...
    void paintTextItem(Graphics g, HtmlItem item, Line line) {
        ...
        if (item.text == null) {
            ...
            g.drawString(item.text, item.xpos, line.ypos - nPosY, Graphics.LEFT | Graphics.BOTTOM);
        }
    }
    ...
    void paintInputItem(Graphics g, HtmlItem item, Line line) {
        ...
        if (w == null) {
            // this is an error
            g.setColor(RED);
            g.drawString("NULL_WIDGET_ERR", item.xpos, line.ypos - nPosY, Graphics.LEFT | Graphics.BOTTOM);
            return;
        }
    ...
    static final String BROKEN_IMAGE = "/broken.gif";
    ...
    */
    ...
    void fetchLink() {
        if (selectedHyperlink != -1) {
            ...
            String val = target.toLowerCase();
            if (target.endsWith(".jpg") || target.endsWith(".gif") || target.endsWith(".png")) {
                isLinkImage = true;
            }
            getURL(target);
        }
    ...
    */
    ...
    public Image readImageFromURL(String url) {
        ...
        Image image;
        try {
            // Check in cache image table
            ImageCache r = (ImageCache) cacheImageTable.get(url);
            if (r != null) {
                buf = r.getImageData();
                image = Image.createImage(r.getImageData(), 0, r.getImageData().length);
            } else {
                // Try to get it from the image store
                ImageStore imageStore = ImageStore.getInstance();
                OAbstractRecord[] images = imageStore.getAll(null, new ImageFilter(url));
                if (images != null && images.length > 0) {
                    ImageRecord rec = (ImageRecord) images[0];
                    buf = rec.getImageData();
                    image = Image.createImage(rec.getImageData(), 0, rec.getImageData().length);
                } else {
                    buf = getURLDataBytes(url);
                    image = Image.createImage(buf, 0, buf.length);

                    ImageRecord newImage = new ImageRecord();
                    newImage.setImageData(buf);
                    newImage.setName(url);
                    imageStore.addRecord(newImage);
                }
            }
        } catch (Exception e) {
            try {
                buf = localGet(BROKEN_IMAGE);
                image = Image.createImage(buf, 0, buf.length);
                //image = Image.createImage(BROKEN_IMAGE);
        ...
        if (buf != null) {
            ...
            imageCache.setImageData(buf);
            imageTable.put(url, imageCache);
        }
        return image;
    }
    ...
    public Hashtable getImageTable() {
        return imageTable;
    }

    public Hashtable getCacheImageTable() {
        return cacheImageTable;
    }

    public void setCacheImageTable(Hashtable cacheImageTable) {
        this.cacheImageTable = cacheImageTable;
    }

    public void addCacheImage(ImageCache image) {
        cacheImageTable.put(image.getUrl(), image);
    }
