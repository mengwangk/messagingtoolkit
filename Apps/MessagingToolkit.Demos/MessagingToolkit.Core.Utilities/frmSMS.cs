    ...
    public partial class frmSMS : Form
    {
        ...
        private void frmSMS_Load(object sender, EventArgs e)
        {
            ...
            if (portNames.Length > 0)
            {
                ...
                var sortedList = portNames;

                PlatformID id = Environment.OSVersion.Platform;
                if (id == PlatformID.Win32Windows || id == PlatformID.Win32NT)
                {
                    // This is a Windows based OS, serial port should start with COM
                    portNames.OrderBy(port => Convert.ToInt32(port.Replace("COM", string.Empty).Replace("c", string.Empty)));
                }
        ...
        private void btnSendvCard_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    txtvCardDestinationNumber.Focus();
                }
            }
            finally
            {
        ...
        private void btnGetCurrentCalls_Click(object sender, EventArgs e)
        {
            CurrentCallInformation[] currentCalls = mobileGateway.GetCurrentCalls();
            if (currentCalls.Length > 0)
            {
                MessageBox.Show(currentCalls.Length + " current call(s) in progress!", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
            } else
            {
                MessageBox.Show("No current call in progress!", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            //mobileGateway.CallControl(CallControlBehavior.PlaceActiveOnHold);
        }
