...
namespace MessagingToolkit.QRCode.Codec.Reader.Pattern
{
	...
	public class FinderPattern
	{
		...
		internal static Line[] FindLineAcross(bool[][] image)
		{
			...
			canvas.DrawLines(foundLines, MessagingToolkit.QRCode.Helper.ColorFields.LIGHTGREEN);
			return foundLines;
		}
		...
		{
			List<Line> crossLines = new List<Line>(10);
			List<Line>  lineNeighbor = new List<Line>(10);
			List<Line> lineCandidate = new List<Line>(10);
			Line compareLine;

            lineCandidate.AddRange(lineAcross);

            List<Line> lineRemove = new List<Line>(10);
			...
			*/
			...
			for (int i = 0; i < lineCandidate.Count - 1; i++)
			{
				...
				{
                    if (lineRemove.Contains(lineCandidate[j])) 
                        continue;
					if (Line.IsNeighbor(lineNeighbor[lineNeighbor.Count - 1], lineCandidate[j]))
					{
						lineNeighbor.Add(lineCandidate[j]);
						compareLine = lineNeighbor[lineNeighbor.Count - 1];
						if (lineNeighbor.Count * 5 > compareLine.Length && j == lineCandidate.Count - 1)
						{
							crossLines.Add(lineNeighbor[lineNeighbor.Count / 2]);

                            // Commented 21 Jan 2012
							for (int k = 0; k < lineNeighbor.Count; k++) {
								lineRemove.Add(lineNeighbor[k]);
                                //lineCandidate.Remove(lineNeighbor[k]);
                            }
						}
					}
					//terminate comparison if there are no possibility for found neighbour lines
					else if (CantNeighbor(lineNeighbor[lineNeighbor.Count - 1], lineCandidate[j]) || (j == lineCandidate.Count - 1))
					{
		...
		internal static int[] GetAngle(Point[] centers)
		{
			...
			canvas.Print("originPoint is: " + originPoint);
		...
		internal static Point[] GetCenter(Line[] crossLines)
		{
			...
			{
				canvas.DrawPolygon(foundPoints, MessagingToolkit.QRCode.Helper.ColorFields.RED);
				return foundPoints;
			}
			else
		...
		internal static Point[] Sort(Point[] centers, int[] angle)
		{
			...
			int quadant = GetURQuadrant(angle);
			switch (quadant)
			{
		...
		internal static int CalcExactVersion(Point[] centers, int[] angle, int[] moduleSize, bool[][] image)
		{
			...
			canvas.DrawPoints(points, MessagingToolkit.QRCode.Helper.ColorFields.RED);
