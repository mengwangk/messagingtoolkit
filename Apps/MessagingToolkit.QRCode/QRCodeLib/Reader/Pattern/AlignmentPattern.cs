...
namespace MessagingToolkit.QRCode.Codec.Reader.Pattern
{
	...
	public class AlignmentPattern
	{
		...
		internal static Point[][] getCenter(bool[][] image, FinderPattern finderPattern, Point[][] logicalCenters)
		{
			...
			canvas.DrawCross(centers[0][0], MessagingToolkit.QRCode.Helper.ColorFields.BLUE);
			
			axis.Origin = finderPattern.GetCenter(FinderPattern.UR);
			centers[sqrtCenters - 1][0] = axis.translate(- 3, 3);
			canvas.DrawCross(centers[sqrtCenters - 1][0], MessagingToolkit.QRCode.Helper.ColorFields.BLUE);
			
			axis.Origin = finderPattern.GetCenter(FinderPattern.DL);
			centers[0][sqrtCenters - 1] = axis.translate(3, - 3);
			canvas.DrawCross(centers[0][sqrtCenters - 1], MessagingToolkit.QRCode.Helper.ColorFields.BLUE);
			...
			for (int y = 0; y < sqrtCenters; y++)
			{
				for (int x = 0; x < sqrtCenters; x++)
				{
					...
					if (y == 0)
					{
						...
						canvas.DrawCross(centers[x][y], MessagingToolkit.QRCode.Helper.ColorFields.RED);
					}
					else if (x == 0)
					{
					...
					canvas.DrawCross(centers[x][y], MessagingToolkit.QRCode.Helper.ColorFields.BLUE);
					canvas.DrawLine(new Line(tmpPoint, centers[x][y]), MessagingToolkit.QRCode.Helper.ColorFields.LIGHTBLUE);
