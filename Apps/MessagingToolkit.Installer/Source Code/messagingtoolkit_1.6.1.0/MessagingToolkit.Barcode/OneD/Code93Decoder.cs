...
namespace MessagingToolkit.Barcode.OneD
{
    ...
    public sealed class Code93Decoder : OneDDecoder
    {
        ...
        public override Result DecodeRow(int rowNumber, BitArray row, Dictionary<DecodeOptions, object> decodingOptions)
        {
            ...
            return new Result(resultString, null, new ResultPoint[] { new ResultPoint(left, (float)rowNumber), new ResultPoint(right, (float)rowNumber) }, BarcodeFormat.Code93);
        ...
        private static int[] FindAsteriskPattern(BitArray row)
        {
            ...
            for (int i = rowOffset; i < width; i++)
            {
                ...
                else
                {
                    ...
                    {
                        if (ToPattern(counters) == AsteriskEncoding)
                        {
