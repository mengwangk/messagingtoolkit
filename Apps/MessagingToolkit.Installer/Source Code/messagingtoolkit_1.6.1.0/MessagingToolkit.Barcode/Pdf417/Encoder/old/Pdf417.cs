...
namespace MessagingToolkit.Barcode.Pdf417.Encoder
{
    ...
    internal sealed class Pdf417
    {
        ...
        /// </summary>
        private static readonly int[][] CodeWordTable = {
				new int[] { 0x1d5c0, 0x1eaf0, 0x1f57c, 0x1d4e0, 0x1ea78, 0x1f53e,
				...
				new int[] { 0x1f560, 0x1fab8, 0x1ea40, 0x1f530, 0x1fa9c, 0x1ea20,
				...
				new int[] { 0x1abe0, 0x1d5f8, 0x153c0, 0x1a9f0, 0x1d4fc, 0x151e0,
        ...
        private const float DefaultModuleWidth = 0.357f; //1px in mm
        private const float Height = 16.0f; //mm

        private int errorCorrectionLevel;
        private BarcodeMatrix barcodeMatrix;

        internal BarcodeMatrix GetBarcodeMatrix()
        {
        ...
        {
            int k = MessagingToolkit.Barcode.Pdf417.Encoder.Pdf417ErrorCorrection.GetErrorCorrectionCodewordCount(errorCorrectionLevel);
        ...
        private void EncodeLowLevel(String fullCodewords, int c, int r, int errorCorrectionLevel, BarcodeMatrix logic)
        {
            ...
            for (int y = 0; y < r; y++)
            {
                int cluster = y % 3;
                logic.StartRow();
                EncodeChar(StartPattern, 17, logic.GetCurrentRow());
                ...
                int pattern = CodeWordTable[cluster][left];
                EncodeChar(pattern, 17, logic.GetCurrentRow());

                for (int x = 0; x < c; x++)
                {
                    pattern = CodeWordTable[cluster][fullCodewords[idx]];
                    EncodeChar(pattern, 17, logic.GetCurrentRow());
                    idx++;
                }
                ...
                EncodeChar(StopPattern, 18, logic.GetCurrentRow());
            }
        }
        ...
        internal void GenerateBarcodeLogic(String msg, int errorCorrectionLevel_0)
        {

            //1. step: High-level encoding
            int errorCorrectionCodeWords = MessagingToolkit.Barcode.Pdf417.Encoder.Pdf417ErrorCorrection.GetErrorCorrectionCodewordCount(errorCorrectionLevel_0);
            String highLevel = MessagingToolkit.Barcode.Pdf417.Encoder.Pdf417HighLevelEncoder.EncodeHighLevel(msg);
            ...
            if (dimension == null)
            {
                throw new BarcodeEncoderException("Unable to fit message in columns");
            }
            ...
            int n = GetNumberOfDataCodewords(sourceCodeWords, errorCorrectionLevel_0,
                    cols);
            if (n > 929)
            {
                throw new BarcodeEncoderException(
            ...
            String ec = MessagingToolkit.Barcode.Pdf417.Encoder.Pdf417ErrorCorrection.GenerateErrorCorrection(dataCodewords, errorCorrectionLevel_0);
            String fullCodewords = dataCodewords + ec;

            //4. step: low-level encoding
            barcodeMatrix = new BarcodeMatrix(rows, cols);
            EncodeLowLevel(fullCodewords, cols, rows, errorCorrectionLevel_0,
                    barcodeMatrix);
        }
        ...
        internal int[] DetermineDimensions(int sourceCodeWords)
        {

            float ratio = 0.0f;
            int[] dimension = null;
            int errorCorrectionCodeWords = MessagingToolkit.Barcode.Pdf417.Encoder.Pdf417ErrorCorrection.GetErrorCorrectionCodewordCount(errorCorrectionLevel);
            ...
            for (int cols = MinCols; cols <= MaxCols; cols++)
            {
                ...
                float newRatio = ((17 * cols + 69) * DefaultModuleWidth)
                        / (rows * Height);
