...
namespace MessagingToolkit.Barcode.Provider
{
    ...
    public sealed class EpsProvider : IOutputProvider<Eps>
    {
        ...
        public Eps Generate(BitMatrix bitMatrix, BarcodeFormat format, string content, Dictionary<EncodeOptions, object> encodingOptions)
        {
            Eps eps = new Eps();
            Create(eps, bitMatrix, format, content, encodingOptions);
            return eps;
        }

        private void Create(Eps image, BitMatrix matrix, BarcodeFormat format, string content, Dictionary<EncodeOptions, object> options)
        {
            if (matrix == null)
                return;
