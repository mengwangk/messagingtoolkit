...
namespace MessagingToolkit.Core.Smpp.Packet.Request
{
	...
	public class SmppSubmitSm : MessageLcd2
	{
		...
		public bool MoreMessagesToSend
		{
			get
			{
				byte[] bytes = GetOptionalParamBytes((ushort)OptionalParamCodes.MoreMessagesToSend);
		...
		public string UssdServiceOp
		{
			get
			{
				return GetOptionalParamString((ushort)OptionalParamCodes.UssdServiceOp);
			}
			...
			set
			{
				if(value == null)
				{
					SetOptionalParamString(
						(ushort)PDU.OptionalParamCodes.UssdServiceOp, string.Empty);
				}
				else if(value.Length == 1)
				{
					SetOptionalParamString(
						(ushort)PDU.OptionalParamCodes.UssdServiceOp, value);
				}
				else
				{
		...
		{
			base.InitPdu();
			DestinationAddressTon = TonType.International;
			DestinationAddressNpi = NpiType.ISDN;
			DestinationAddress = null;
			CommandStatus = 0;
			CommandID = CommandIdType.SubmitSm;
		}
		...
		protected override void DecodeSmscResponse()
		{
			...
			ShortMessage = SmppStringUtil.GetStringFromBody(ref remainder, 5, 5 + _SmLength, DataCoding);
