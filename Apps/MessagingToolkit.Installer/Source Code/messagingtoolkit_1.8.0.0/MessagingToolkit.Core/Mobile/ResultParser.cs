...
namespace MessagingToolkit.Core.Mobile
{
    ...
    internal static class ResultParser
    {
        ...
        public static string ParseError(ICommand command, string response)
        {
            ...
            {
                Match match = Regex.Match(response, StringEnum.GetStringValue(Response.CmsError));
                return GetCmsErrorDescription(match.Groups[1].Value);
                /*
            ...
            {
                Match match = Regex.Match(response, StringEnum.GetStringValue(Response.CmeError));
                return GetCmeErrorDescription(match.Groups[1].Value);
                /*
                string[] cols = response.Split(':');
