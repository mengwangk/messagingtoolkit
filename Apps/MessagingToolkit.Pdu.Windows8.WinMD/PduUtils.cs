...
namespace MessagingToolkit.Pdu
{
    ...
    /// PDU utility class
    /// </summary>
    internal sealed class PduUtils
    {
        /// <summary>
        ...
        /// </summary>
        internal const int ReplyAddressElement = 0x22;
        ...
        public static int GetAddressTypeFor(string address)
        {
            ...
            if (!international) // Modified: May 18 2012
            {
                for (int i = 0; i < address.Length; i++)
                {
                    if (!Char.IsDigit(address[i]))
                    {
                        ...
                        return CreateAddressType((int)AddressType.AddressTypeAlphanumeric);
                    }
                }
            }
            ...
            if (international)
            {
                return CreateAddressType((int)AddressType.AddressTypeInternational | (int)AddressNumberPlan.AddressNumberPlanIdTelephone);
            }
            else
            {
                return CreateAddressType((int)AddressType.AddressTypeUnknown | (int)AddressNumberPlan.AddressNumberPlanIdTelephone);
            }
        }

        public static int ExtractAddressType(int addressType)
        {
            return addressType & (int)AddressType.AddressTypeMask;
        }

        public static int ExtractNumberPlan(int addressType)
        {
            return addressType & (int)AddressNumberPlan.AddressNumberPlanIdMask;
        }
        ...
        {
            return dataCodingScheme & ~(int)DcsEncoding.DcsEncodingMask;
        }

        public static int ExtractDcsFlash(int dataCodingScheme)
        {
            // this is only useful if DCS != 0
            return dataCodingScheme & ~(int)DcsMessageClass.DcsMessageClassMask;
        }

        public static int ExtractDcsCodingGroup(int dataCodingScheme)
        {
            return dataCodingScheme & ~(int)DcsCodingGroup.DcsCodingGroupMask;
        }
        ...
        public static string DecodeFirstOctet(Pdu pdu)
        {
            ...
            if ((HasTpField(pdu, "TpVpf")) && ((Object)HasTpField(pdu, "TpVpf") != null))
            {
                switch (GetTpField(pdu, "TpVpf"))
                {
                    ...
                    case (int)ValidityPeriodFormat.TpVpfNone:
        ...
        public static string DecodeDataCodingScheme(Pdu pdu)
        {
            ...
            if ((pdu.DataCodingScheme & ~(int)DcsCodingGroup.DcsCodingGroupGeneral) == 0)
            {
                if (PduUtils.ExtractDcsFlash(pdu.DataCodingScheme) == (int)DcsMessageClass.DcsMessageClassFlash)
                {
