        ...
        */
		...
		public Pdu ParsePdu(string rawPdu)
		{
            ...
            try
            {
                ...
                if (pdu.SmscAddressType != (int)AddressType.AddressTypeInternationFormat &&
                    pdu.SmscAddressType != (int)AddressType.AddressTypeDomesticFormat &&
                    pdu.SmscAddressType != (int)AddressType.AddressTypeUnknown)
                {
                    position = 0;
                    // parse start and determine what type of pdu it is
		...
		private void  ParseSmsSubmitMessage(SmsSubmitPdu pdu)
		{
			...
			switch (pdu.TpVpf)
			{
                ...
                case (int)ValidityPeriodFormat.TpVpfInteger: 
					...
					pdu.ValidityPeriod = validityInt / 60; // pdu assumes hours
					break;
