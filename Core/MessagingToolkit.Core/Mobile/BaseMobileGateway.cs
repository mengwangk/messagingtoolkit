...
//#endif
        ...
        public virtual int ReadTimeout
        {
            get
            {
                if (Port == null) return config.ReadTimeOut;
        ...
        public virtual int WriteTimeout
        {
            get
            {
                if (Port == null) return config.WriteTimeOut;
       ...
       // }
        ...
        /// <summary>
        /// Enable list current calls feature. 
        /// May not be supported on all modems. Use <see cref="GetCurrentCalls">GetCurrentCalls</see> if this API
        /// does not work.
        /// </summary>
        /// <returns>true if successful, false otherwise</returns>
        public bool EnableCallListingNotifications()
        {
            ClccFeature feature = GetFeature<ClccFeature>(ClccFeature.NewInstance());
            try
            {
                feature.Mode = CapabilityMode.On;
                IContext context = Execute(feature);
        ...
        /// <summary>
        /// Disable list current calls feature. 
        /// May not be supported on all modems. Use <see cref="GetCurrentCalls">GetCurrentCalls</see> if this API
        /// does not work.
        /// </summary>
        /// <returns>true if successful, false otherwise</returns>
        public bool DisableCallListingNotifications()
        {
            ClccFeature feature = GetFeature<ClccFeature>(ClccFeature.NewInstance());
            try
            {
        ...
        /// <summary>
        ///  Get a list of current call of the modem, if any
        ///  <returns>A list of current call</returns>
        /// </summary>
        public virtual CurrentCallInformation[] GetCurrentCalls()
        {            
            try
            {
                GetCurrentCallsFeature feature = GetFeature<GetCurrentCallsFeature>(GetCurrentCallsFeature.NewInstance());
                IContext context = Execute(feature);
                return (CurrentCallInformation[])context.GetResult();
            }
            catch (Exception ex)
            {
                this.exception = ex;
            ...
            return new CurrentCallInformation[] { };
        }

        /// <summary>
        /// Control the current ongoing calls
        /// </summary>
        /// <param name="behavior">Call control behavior</param>
        /// <param name="index">The current call index retrieved using <see cref="GetCurrentCalls">GetCurrentCalls</see></param>
        /// <returns>true if successful</returns>
        public virtual bool CallControl(CallControlBehavior behavior, int index)
        {
            try
            {
                // Control call
                CallControlFeature feature = GetFeature<CallControlFeature>(CallControlFeature.NewInstance());
        ...
        /// <param name="behavior">Call control behavior</param>
        /// <returns>true if successful</returns>
        public virtual bool CallControl(CallControlBehavior behavior)
        {
            return CallControl(behavior, -1);
        }
        ...
        */
                ...
                if (!string.IsNullOrEmpty(line))
                {
                    ...
                    if (handlers.IsUnsolicitedCall(data))
                    {
                        ...
                        if (callInformation != null && callInformation.GetType() == typeof(CurrentCallInformation))
                        {
                            ...
                            ProcessCallListingReceived(callInformation);
                        }
                        else
                        {
        ...
        private void SendMms(Mms message)
        {
            try
            {
                ...
                if (config.UseHttpTransportForMMS)
                {
                    SendMmsV2Feature feature = GetFeature<SendMmsV2Feature>(SendMmsV2Feature.NewInstance());
                    feature.Message = message;
                    ...
                    feature.SupportHttpCharsetHeader = this.config.SupportHttpCharsetHeader;
                    IContext context = Execute(feature);
                }
                else
                {
        ...
        private bool RetrieveMms(MessageInformation messageInformation, ref Mms message)
        {
            try
            {
                if (config.UseHttpTransportForMMS)
                {
                    RetrieveMmsV2Feature feature = GetFeature<RetrieveMmsV2Feature>(RetrieveMmsV2Feature.NewInstance());
        ...
        private void ProcessCallListingReceived(IIndicationObject e)
        {
            this.statistics.IncomingCall++;
            if (this.CallReceived != null)
            {
                CurrentCallInformation currentCallInformation = (CurrentCallInformation)e;
                currentCallInformation.GatewayId = this.Id;
                CallInformation callInformation = new CallInformation(currentCallInformation.Number, currentCallInformation.NumberType, DateTime.Now, currentCallInformation.GatewayId);

                if (!string.IsNullOrEmpty(currentCallInformation.Number))  // Raise event if calling number is not empty
                {
                    // ****************** TODO - to fire call listing event **************************** 
                    // Logger.LogThis("Incoming call listing received. Currently this event is not triggered.", LogLevel.Verbose, this.Id);

                    IncomingCallEventArgs incomingCallEventArgs = new IncomingCallEventArgs(callInformation);
                    incomingCallEventArgs.CurrentCallInformation = currentCallInformation;
                    this.CallReceived.BeginInvoke(this, incomingCallEventArgs, new AsyncCallback(this.AsyncCallback), null);
                }
            }
