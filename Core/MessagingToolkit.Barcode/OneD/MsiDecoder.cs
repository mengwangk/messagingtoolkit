...
namespace MessagingToolkit.Barcode.OneD
{
    ...
    public sealed class MsiDecoder : OneDDecoder
    {
        ...
        /// </summary>
        internal static int[] CharacterEncodings = { 0x924, 0x926, 0x934, 0x936, 0x9A4, 0x9A6, 0x9B4, 0x9B6, 0xD24, 0xD26 };

        private const int StartEncoding = 0x06;
        private const int EndEncoding = 0x09;
        ...
        override public Result DecodeRow(int rowNumber, BitArray row, Dictionary<DecodeOptions, object> decodingOptions)
        {
            ...
            return new Result(resultString, rawBytes, new[] { new ResultPoint(left, rowNumber), new ResultPoint(right, rowNumber) }, BarcodeFormat.MSIMod10);
        }
                ...
                else
                {
                    if (counterPosition == patternLength - 1)
                    {
                        ...
                        if (factorNarrowToWide >= 1.5 && factorNarrowToWide <= 5)
                        {
                            ...
                            if (ToPattern(counters, patternLength) == StartEncoding)
                            {
            ...
            for (int i = rowOffset; i < width; i++)
            {
                ...
                else
                {
                    if (counterPosition == patternLength - 1)
                    {
                        ...
                        if (factorNarrowToWide >= 1.5 && factorNarrowToWide <= 5)
                        {
                            if (ToPattern(counters, patternLength) == EndEncoding)
                            {
