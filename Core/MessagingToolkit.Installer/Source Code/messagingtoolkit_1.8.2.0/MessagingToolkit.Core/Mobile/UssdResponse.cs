	...
	public class UssdResponse : UssdDatagram, IIndicationObject
	{
		...
		{           
			...
			try
			{
                ...
                int status;
                bool isNumeric = int.TryParse(matcher.Groups[StatusIndex].Value, out status);
                if (isNumeric)
				    this.SessionStatus = UssdSessionStatus.GetByNumeric(status);
				...
				{
                    int dcs;
                    isNumeric = int.TryParse(matcher.Groups[EncodingIndex].Value, out dcs);
                    if (isNumeric)
					    this.Dcs = UssdDcs.GetByNumeric(dcs);				   
				}
			}
			catch (Exception e)
			{
