...
namespace MessagingToolkit.Barcode.OneD
{
    ...
    {
        private Dictionary<char, string> C39_Code = new Dictionary<char, string>(); //is initialized by init_Code39()
        ...
        private string Encode_Code39()
        {
            ...
            string strFormattedData = "*" + rawData.Replace("*", "") + "*";
            ...
            foreach (char c in strFormattedData)
            {
                ...
                catch
                {
                    if (_AllowExtended)
                        ...
                        Error("Invalid data. (Try using Extended Code39)");
                }//catch
            }//foreach

            result = result.Substring(0, result.Length-1);

            //clear the dictionary so it no longer takes up memory
            this.C39_Code.Clear();
