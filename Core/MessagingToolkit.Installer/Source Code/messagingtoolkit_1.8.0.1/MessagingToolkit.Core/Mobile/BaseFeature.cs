...
namespace MessagingToolkit.Core.Mobile
{
    ...
    internal abstract class BaseFeature<T>
    {
        ...
        /// </summary>
        private int DefaultWaitForResponseInterval = 300;

        /// <summary>
        /// Number of retries to wait for response
        /// </summary>
        private int DefaultWaitForResponseRetryCount = 100;
        ...
        protected BaseFeature()
        {
            ...
            WaitForResponseInterval = DefaultWaitForResponseInterval;

            // Default to 100 times
            WaitForResponseRetryCount = DefaultWaitForResponseRetryCount;
        ...
        /// <summary>
        /// Interval in milliseconds to wait for the command response.
        /// </summary>
        ...
        /// <summary>
        /// Number of retry to wait for the response.
        /// </summary>
        public int WaitForResponseRetryCount
        {
            get;
            set;
        }
        ...
        {            
            ...
            try
            {
                ...
                while (IncomingDataQueue.Count == 0) 
                {
                    ...
                    if (count++ == WaitForResponseRetryCount) break;
                }
                //if (Port.BytesToRead > 0)
                if (IncomingDataQueue.Count > 0) 
                {
                    count = 0;
                    while (count++ <= WaitForResponseRetryCount)
                    {
                        ...
                        while (IncomingDataQueue.Count > 0)
                        {
                            ...
                            string data = IncomingDataQueue.Dequeue();
                            ...
                            if (string.IsNullOrEmpty(data))
                            {
                                ...
                                // Remove it from queue - Oct 11 2014
        ...
        override public string ToString()
        {
            ...
            str = String.Concat(str, "IncomingDataQueue = ", IncomingDataQueue, "\r\n");
            str = String.Concat(str, "Supported = ", Supported, "\r\n");
            str = String.Concat(str, "ExecutionBehavior = ", ExecutionBehavior, "\r\n");
            str = String.Concat(str, "StopExecution = ", StopExecution, "\r\n");
            str = String.Concat(str, "Commands = ", Commands, "\r\n");
            str = String.Concat(str, "CommandType = ", CommandType, "\r\n");
            str = String.Concat(str, "HasEcho = ", HasEcho, "\r\n");
            str = String.Concat(str, "RequiredReset = ", RequiredReset, "\r\n");
            str = String.Concat(str, "WaitForResponseInterval = ", WaitForResponseInterval, "\r\n");
            str = String.Concat(str, "WaitForResponseRetryCount = ", WaitForResponseRetryCount, "\r\n");
            return str;
        }
