...
namespace MessagingToolkit.Core.Smpp.Utility
{
	...
	public class TlvTable
	{
		...
		private void InjectTlv(byte[] tlvData, ref Int32 index)
		{
            ...
            {
                Logger.LogThis("tag bytes " + temp[0].ToString("X").PadLeft(2, '0') +
                                  temp[1].ToString("X").PadLeft(2, '0'), LogLevel.Verbose);
            }
			...
			Logger.LogThis("TLV Length " + length, LogLevel.Verbose);
