...
namespace MessagingToolkit.Core.Mobile
{
    ...
    internal class MessageIndicationHandlers
    {
        ...
        private const string DeliverMemoryIndication5 = "\\+CMTI: \"(\\w+)\", (\\d+)";
        ...
        private const string UssdResponseIndication = "^\\+CUSD:\\s+(\\d)(?:,\\s*\"([^\"]*))?(?:\",\\s*(\\d+)\\s*)?\"?\r?$";
        //private const string UssdResponseIndication = "\\+CUSD:\\s*(\\d+),\\s*\"([^\"]*)\"(,\\s*(\\d+))*";
        //private const string UssdResponseIndicationStart = @"\+CUSD:";
        // Modified Oct 15 2013
        private const string UssdResponseIndicationStart = "^\\+CUSD:\\s+(\\d)";
        ...
        public MessageIndicationHandlers()
        {
            ...
            UnsolicitedMessage item5 = new UnsolicitedMessage(DeliverMemoryIndication5, new UnsolicitedHandler(this.HandleDeliverMemoryIndication));
            item5.StartPattern = DeliverMemoryIndicationStart;
            item5.Description = "New SMS-DELIVER received (indicated by memory location)";
            this.messages.Add(item5);
            ...
            if (!match.Success)
            {
                ...
                if (!match.Success)
                {
                    ...
                    if (!match.Success)
                    {
                        ...
                        if (!match.Success)
                        {
                            match = new Regex(DeliverMemoryIndication5).Match(input);
                            if (!match.Success)
