        ...
        **/
        ...
        private void lvwMessageThreads_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(threadId))
            {
                ...
                AppendMessageLog(string.Format("Retrieving messages for thread {0}", threadId));
        ...
        private void chkMarkReadMessages_CheckedChanged(object sender, EventArgs e)
        {
            if (HttpGatewayFactory.IsDefaultOrNull(httpGateway))
            {
                MessageBox.Show("Make sure device is connected before selecting this option", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                chkMarkReadMessages.CheckedChanged -= chkMarkReadMessages_CheckedChanged;
                chkMarkReadMessages.Checked = false;
                chkMarkReadMessages.CheckedChanged += chkMarkReadMessages_CheckedChanged;
                return;
            }
            httpGateway.Configuration.MarkReadMessage = chkMarkReadMessages.Checked;
        }
    }
}
