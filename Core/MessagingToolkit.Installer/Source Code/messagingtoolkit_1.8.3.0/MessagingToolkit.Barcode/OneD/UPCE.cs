...
namespace MessagingToolkit.Barcode.OneD
{
    class UPCE : BarcodeCommon, IBarcode
    {
        ...
        {
            if (rawData.Length != 6 && rawData.Length != 8 && rawData.Length != 12) Error("Invalid data length. (8 or 12 numbers only)");

            if (!MessagingToolkit.Barcode.BarcodeEncoder.CheckNumericOnly(rawData)) Error("Numeric only.");

            int CheckDigit = Int32.Parse(rawData[rawData.Length - 1].ToString());
            int NumberSystem = Int32.Parse(rawData[0].ToString());
            ...
            if (rawData.Length == 12)
            {
                ...
                if (NumberSystem != 0 && NumberSystem != 1)
                    Error("Invalid Number System (only 0 & 1 are valid)");
                ...
                else if (!Manufacturer.EndsWith("0") && Int32.Parse(ProductCode) <= 9 && Int32.Parse(ProductCode) >= 5)
                {
                    ...
                    Error("Illegal UPC-A entered for conversion.  Unable to convert.");
            ...
            {
                int i = Int32.Parse(rawData[pos++].ToString());
                if (c == 'a')
                {
                    result += EAN_CodeA[i];
                }//if
