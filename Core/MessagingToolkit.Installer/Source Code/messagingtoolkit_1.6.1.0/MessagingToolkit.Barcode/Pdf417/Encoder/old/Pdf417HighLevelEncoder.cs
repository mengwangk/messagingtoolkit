...
namespace MessagingToolkit.Barcode.Pdf417.Encoder
{
    ...
    internal sealed class Pdf417HighLevelEncoder
    {
        ...
        static internal String EncodeHighLevel(String msg, bool byteCompaction)
        {
            ...
            if (byteCompaction)
            { // Can choose only byte compaction
                ...
                while (p < len)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            int b_0 = DetermineConsecutiveBinaryCount(msg, bytes, p);
                            if (b_0 == 0)
                            {
                                b_0 = 1;
                            }
                            if (b_0 == 1 && encodingMode == TEXT_COMPACTION)
                            {
                                //Switch for one byte (instead of latch)
                                EncodeBinary(bytes, p, 1, TEXT_COMPACTION, sb);
                            }
                            ...
                            {
                                //Mode latch performed by encodeBinary()
                                EncodeBinary(bytes, p, b_0, encodingMode, sb);
                                encodingMode = BYTE_COMPACTION;
                                textSubMode = SUBMODE_ALPHA; //Reset after latch
                            }
                            p += b_0;
        ...
        private static void EncodeNumeric(String msg, int startpos, int count, StringBuilder sb) {
          ...
          bigi num900 = BigInteger.valueOf(900);
                ...
                StringBuilder sb)
        {
            ...
            while (idx < count - 1)
            {
                ...
                String part = '1' + msg.Substring(startpos + idx, (startpos + idx
                                    + len) - (startpos + idx));
                long bigint = ((Int64)Int64.Parse(part, System.Globalization.NumberStyles.Integer));
                do
                {
                    long c = bigint % 900;
                    tmp.Append((char)c);
