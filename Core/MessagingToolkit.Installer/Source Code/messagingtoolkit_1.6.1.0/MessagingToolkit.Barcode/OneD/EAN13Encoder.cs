    ...
    public sealed class EAN13Encoder : UPCEANEncoder
    {
       private const int CODE_WIDTH = 3 + // start guard
		...
		public override bool[] Encode(String contents) {
			...
			for (int i_0 = 7; i_0 <= 12; i_0++) {
				int digit_1 = Int32.Parse(contents.Substring(i_0,(i_0 + 1)-(i_0)));
                pos += MessagingToolkit.Barcode.OneD.OneDEncoder.AppendPattern(result, pos, MessagingToolkit.Barcode.OneD.UPCEANDecoder.LPatterns[digit_1], true);
			}
            pos += MessagingToolkit.Barcode.OneD.OneDEncoder.AppendPattern(result, pos, MessagingToolkit.Barcode.OneD.UPCEANDecoder.StartEndPattern, true);
