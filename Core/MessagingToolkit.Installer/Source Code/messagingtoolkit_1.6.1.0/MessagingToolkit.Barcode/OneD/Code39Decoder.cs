...
namespace MessagingToolkit.Barcode.OneD
{
    ...
    public sealed class Code39Decoder : OneDDecoder
    {
		...
		private static readonly int ASTERISK_ENCODING = CharacterEncodings[39];
        ...
        public override Result DecodeRow(int rowNumber, BitArray row, Dictionary<DecodeOptions, object> decodingOptions)
        {
			...
			/* foreach */
			foreach (int counter_0  in  counters) {
				lastPatternSize += counter_0;
			}
			...
			if (usingCheckDigit) {
				...
				int total = 0;
				for (int i = 0; i < max; i++) {
					total += AlphabetString.IndexOf(result[i]);
				}
			...
			return new Result(resultString, null, new ResultPoint[] { new ResultPoint(left, (float) rowNumber), new ResultPoint(right, (float) rowNumber) }, BarcodeFormat.Code39);
		...
		private static int ToNarrowWidePattern(int[] counters) {
			...
			do {
				...
				for (int i = 0; i < numCounters; i++) {
					int counter_0 = counters[i];
					if (counters[i] > maxNarrowCounter) {
						pattern |= 1 << (numCounters - 1 - i);
						wideCounters++;
						totalWideCountersWidth += counter_0;
					}
				}
				if (wideCounters == 3) {
					...
					for (int i_1 = 0; i_1 < numCounters && wideCounters > 0; i_1++) {
						int counter_2 = counters[i_1];
						if (counters[i_1] > maxNarrowCounter) {
							wideCounters--;
							// totalWideCountersWidth = 3 * average, so this checks if counter >= 3/2 * average
							if ((counter_2 << 1) >= totalWideCountersWidth) {
								return -1;
							}
						}
					}
