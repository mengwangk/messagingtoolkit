...
namespace MessagingToolkit.Barcode.OneD.Rss
{
    ...
    public sealed class Rss14Decoder : AbstractRssDecoder
    {
        ...
        public override Result DecodeRow(int rowNumber, BitArray row, Dictionary<DecodeOptions, object> decodingOptions)
        {
            ...
            {
                Pair left = (Pair)possibleLeftPairs[l];
                if (left.Count > 1)
                {
                    for (int r = 0; r < numRightPairs; r++)
                    {
                        Pair right = (Pair)possibleRightPairs[r];
                        if (right.Count > 1)
                        {
        ...
        private Pair DecodePair(BitArray row, bool right, int rowNumber, Dictionary<DecodeOptions, object> decodingOptions)
        {
            try
            {
