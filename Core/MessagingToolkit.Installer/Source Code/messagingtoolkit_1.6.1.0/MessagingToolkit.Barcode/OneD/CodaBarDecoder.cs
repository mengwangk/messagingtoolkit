    ...
    public sealed class CodaBarDecoder : OneDDecoder
    {
     ...
     // These values are critical for determining how permissive the decoding
		...
		public override Result DecodeRow(int rowNumber, BitArray row, Dictionary<DecodeOptions, object> decodingOptions) {
			...
			int runningCount = 0;
			for (int i_1 = 0; i_1 < startOffset; i_1++) {
				runningCount += counters[i_1];
			}
		...
		private int ToNarrowWidePattern(int position) {
			...
			int bitmask = 1 << 7;
			int pattern = 0;
			for (int i_0 = 0; i_0 < 7; i_0++) {
				int barOrSpace = i_0 & 1;
				bitmask >>= 1;
				if (counters[position + i_0] > thresholds[barOrSpace]) {
					pattern |= bitmask;
				}
			}
	
			for (int i_1 = 0; i_1 < CHARACTER_ENCODINGS.Length; i_1++) {
				if (CHARACTER_ENCODINGS[i_1] == pattern) {
					return i_1;
				}
			}
			return -1;
