...
namespace MessagingToolkit.Pdu.WapPush
{
	...
	public class WapSiUserDataGenerator
	{
		...
		virtual public WapSiPdu WapSiPdu
		{
			set
			{
				this.pdu = value;
		...
		public virtual byte[] GenerateWapSiUDBytes()
		{
			try
			{
		...
		private void  WriteWapSiData()
		{
			...
			// created attribute
			DateTime tempAux = pdu.CreateDate;
			WriteCreatedAttribute(ref tempAux);
			// expire attribure
			DateTime tempAux2 = pdu.ExpireDate;
			WriteExpiresAttribute(ref tempAux2);
			// action attribute
			WriteActionAttribute(pdu.WapSignal);
			// si-id attribute
		...
		private void  WriteHrefAttribute(string url)
		{
			...
			{
				throw new SystemException("Invalid URL: '" + url + "'");
			}
		...
		private void  WriteCreatedAttribute(ref DateTime createDate)
		{
			if (createDate != null)
			{
        ...
        private void  WriteExpiresAttribute(ref DateTime expireDate)
		{
			if (expireDate != null)
			{
        ...
        private void  WriteDate(ref DateTime date)
		{
