...
namespace MessagingToolkit.Pdu
{
	public abstract class Pdu
	{
			...
			set
			{
				switch (value & ~ PduUtils.DcsEncodingMask)
				{
					...
					case PduUtils.DcsEncodingUcs2: 
						break;					
					default: 
						throw new SystemException("Invalid encoding value: " + PduUtils.ByteToPdu(value));
		...
		protected internal virtual void  SetFirstOctetField(int fieldName, int fieldValue, int[] allowedValues)
		{
			...
			throw new SystemException("Invalid value for fieldName.");
		}
		...
		protected internal virtual void  CheckTpMti(int allowedType)
		{
			...
			{
				throw new SystemException("Invalid message type : " + TpMti);
			}
		}
			...
			if (!HasTpUdhi())
			{
				switch (udhCheckMode)
				{
					...
					case UdhCheckModeExceptionIfNone: 
						throw new SystemException("PDU does not have a UDHI in the first octet");
					...
					default: 
						throw new SystemException("Invalid UDH check mode");
		...
		private string DecodeNonUdhDataAsString()
		{
			...
			throw new SystemException("Invalid dataCodingScheme: " + DataCodingScheme);
		}
